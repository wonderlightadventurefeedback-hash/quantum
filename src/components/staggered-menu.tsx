            ...
            <ul className="sm-panel-list list-none m-0 p-0 flex flex-col gap-4" role="list" data-numbering={displayItemNumbering || undefined}>
              {(items || []).map((it, idx) => (
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                  <Link className="sm-panel-item relative text-black font-bold text-lg md:text-xl cursor-pointer leading-none tracking-tighter uppercase transition-colors inline-block no-underline pr-12" href={it.link} aria-label={it.ariaLabel} data-index={idx + 1} onClick={closeMenu}>
